...
import get_admins from '@/API/get_admins.js'
import { parseCookies } from 'nookies';

const router = useRouter()
const data = ref([])
const token = parseCookies().lgn


function redirect_params(nama_lengkap,uid) {
...
onMounted (async() => {
  const response = await get_admins(token)
  data.value = response
...
<template>
    <ul class="menu menu-sm  rounded-lg max-w-xs w-full bg-white dark:bg-[#222] ">
        <li><router-link :to="'/'">
          Home
        </router-link></li>
        <li>
          <details open>
            <summary>
              ...
              Laporan berdasarkan nama penulis
            </summary>
            <ul>
              <li v-for="datas in data" :key="datas.id">
                <a @click="redirect_params(datas.nama_lengkap,datas.uid)">
        ...
        <li><a>
          ...
          data-pemasukan-pengeluaran-sumbangan.pdf
        </a></li>
      </ul>
